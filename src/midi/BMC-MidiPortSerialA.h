...
*/
#ifndef BMC_MIDI_SERIAL1_H
#define BMC_MIDI_SERIAL1_H
...
struct midiSerialASettings : public midi::DefaultSettings {
   static const unsigned SysExMaxSize = BMC_MIDI_SYSEX_SIZE;
};

class BMCMidiPortSerialA {
public:
  midi::MidiInterface<HardwareSerial,midiSerialASettings> Port;
  BMCMidiPortSerialA():Port(BMC_MIDI_SERIAL_IO_A){}
  void begin(uint8_t channel=MIDI_CHANNEL_OMNI){
    BMC_PRINTLN("    SerialA BMCMidiPortSerialA::begin");
    Port.begin(channel);
    Port.turnThruOff();
  }
  bool read(BMCMidiMessage& message, bool ignoreRealTime){
    if(Port.read()){
      message.reset(BMC_MIDI_PORT_SERIAL_A_BIT);
