...
BMC24LC256::BMC24LC256(BMC_eeprom_size_t deviceCapacity, byte nDevice, unsigned int pageSize, byte eepromAddr)
{
...
byte BMC24LC256::begin(twiClockFreq_t twiFreq){
...
byte BMC24LC256::write(unsigned long addr, byte *values, unsigned int nBytes){
    byte ctrlByte;       //control byte (I2C device address & chip/block select bits)
    byte txStatus = 0;   //transmit status
...
//a status of EEPROM_ADDR_ERR is returned. For I2C errors, the status
//from the Arduino Wire library is passed back through to the caller.
byte BMC24LC256::read(unsigned long addr, byte *values, unsigned int nBytes)
{
    byte ctrlByte;
    byte rxStatus;
...
//a status of EEPROM_ADDR_ERR is returned. For I2C errors, the status
//from the Arduino Wire library is passed back through to the caller.
byte BMC24LC256::write(unsigned long addr, byte value)
{
