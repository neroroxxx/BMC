...
/*
    ...
    // pot pins
    buff.appendToSysEx8Bits(BMC_MAX_POTS);
    // since the maximum number of mux pins is now 96 we'll have up to 160 pins
    // we have to break each pin number into two 2-bit midi words otherwise
    // the maximum pin number would be 127
    // if somebody has 96 pots that would require a midi sysex of at least 192 bytes
    // we don't want to do this since these midi messages have their bytes allocated
    // in RAM so we'll break pot pins into 3 message for page pots and 3 for global pots
    // this value below will tell the editor that this message contains the last
    // of the pot pins
    uint8_t numOfPinsInMessage = 0;
    for(uint8_t i=0;i<32;i++){
      if(i<BMC_MAX_POTS){
