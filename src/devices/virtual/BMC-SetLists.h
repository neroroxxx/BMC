...
public:
  ...
  void set(uint8_t n){
    ...
    if(setList != n){
      setList = n;
      BMC_PRINT_CHANGE("BMCSetLists setList: ", n);
  ...
  void setSong(uint8_t n){
    ...
    if(song != n || flags.read(BMC_FLAG_SETLISTS_CHANGED)){
      ...
      BMC_PRINT_CHANGE("BMCSetLists song: ", n, " | songInLibrary: ", songInLibrary);
    }
  }
  void setPart(uint8_t n){
    ...
    if(songPart != n || flags.read(BMC_SETLIST_FLAG_PART_RECALL) || flags.read(BMC_FLAG_SETLISTS_SONG_CHANGED)){
      ...
      BMC_PRINT_CHANGE("BMCSetLists songPart: ", songPart);
  ...
  void scrollSet(bool t_direction, bool t_wrap, uint8_t t_amount, uint8_t t_min, uint8_t t_max){
    ...
    uint8_t nextSetList = BMCCycle<uint8_t>(t_min, t_max)
      .withAmount(t_amount)
  ...
  void scrollSong(bool t_direction, bool t_wrap, uint8_t t_amount, uint8_t t_min, uint8_t t_max){
    ...
    uint8_t nextSong = BMCCycle<uint8_t>(t_min, t_max)
      .withAmount(t_amount)
      .withDirection(t_direction)
      .withWrap(t_wrap)
      .withValue(song)
  ...
  void scrollPart(bool t_direction, bool t_wrap, uint8_t t_amount, uint8_t t_min, uint8_t t_max){
    ...
    uint8_t nextPart = BMCCycle<uint8_t>(t_min, t_max)
      .withAmount(t_amount)
      .withDirection(t_direction)
      .withWrap(t_wrap)
      .withValue(song)
