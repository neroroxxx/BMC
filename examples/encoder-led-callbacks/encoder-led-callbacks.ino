...
void setup(){
  ...
  bmc.onEncoderActivity(onEncoderActivity);
...
void onEncoderActivity(uint8_t index, bool increased, bmcStoreEncoder data){
  BMC_PRINT("Encoder", index);
  BMC_PRINT("");
  if(increased){
    BMC_PRINTLN("Increased");
  } else {
    BMC_PRINTLN("Decreased");
  }
}
